...
function OracleConceptBoard() {
  ...
  function deleteDraft(id) {
    const draft = cardDrafts.find((d) => d.id === id);
    const label = draft && draft.name ? `"${draft.name}"` : "this card";
      ...
      {view === "draft" && draftSubView === "list" && (
        <section style={styles.reflectSection}>
          ...
          {cardDrafts.map((d) => (
            ...
            >
              <div style={styles.draftListMain}>
                <span style={styles.draftListName}>{d.name || "Untitled"}</span>
                {d.trigger && (
                  <span style={styles.draftListTrigger}>{d.trigger}</span>
                )}
              </div>
